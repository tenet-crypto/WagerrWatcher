	...
	function api_wgr(){
		...
		$.get(events_url, function(data){
			...
			$.get(action_url, function(data){
				...
				//sort high to low for big bets
				bets_action_sorted = [];
				$.each(bets_action, function(k,v){
					//push first to array
					if(k ==0){
						bets_action_sorted.push(v);
					//sort the rest using the function
					}else{
						var new_position = check_big_bets(v.betValue);
						bets_action_sorted.splice(new_position, 0, v);
					}
				});

					//sort high to low based on bet amount
					function check_big_bets(bet){
						var kk = 0;
						$.each(bets_action_sorted, function(k,v){
								if(bet <= v.betValue){
									if(k >= kk){
										kk = k+1;
									}
								}else if(bet > v.betValue){
									if(k < kk){
										kk = k-1;
									}
								}
				...
				$('#big_bet_placed').html(bets_action_sorted.length);
				...
				$.get(info_url, function(data_2){
					...
					//sort the bets from high to low
					pending_bets_sorted = [];
					$.each(pending_bets, function(k,v){
						//push first to array
						if(k ==0){
